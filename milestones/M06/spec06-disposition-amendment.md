# DRAFT v3 — proposed amendment to SPEC/06's Tier B disposition clause
...
**Revised twice after `pm-spec-reviewer` returned REQUEST CHANGES** — ten
blockers on v1, thirteen on v2. Corrections are marked inline as "v1"/"v2"
rather than edited away: six of the twenty-three were arithmetic that did not
re-derive, in a document whose argument is that figures must, and one was an
argument that contradicted this document's own finding at the exact point where
it asked the seat to act.

**There are SIX proposed changes.** v1 claimed to change only the vehicle and
was wrong; v2 named four and the review found two more. Each is stated as
itself, with its direction:

| # | change | direction on a `keep` verdict |
|---|---|---|
| 1 | the vehicle: an open-loop retrieval driver | see Change 4 — **not derivable** |
| 2 | the clause's surroundings, which break under 1 | none |
| 3 | which samples the dispositive p95 is taken over | none intended; stated |
| 4 | the load each tier receives | **two opposing effects, net sign unknown** |
| 5 | the vantage moves in-region | **unknown**; ADR-0012 flagged it |
| 6 | a floor: a run that reaches no real load is a failed measurement | **toward `keep`**, and bounded |
| — | the error-rate disjunct narrows under the new vehicle | **toward `retire`** |
...
Tier B  3 × 300 s × 170,784 = 153,705,233
                      total = 324,688,133 Opus tokens = 125× the daily cap
...
- **`SPEC/06:52-54`, Out of scope** — must add that the 500-user profile now
  ...
  recorded "corpus size is 49 documents". *v2 cited `:110-112` and the second
...
than from the dev laptop. `ADR-0012:107-110` says Lambda-to-AOSS in-region "has
...
> corpus fingerprint recorded identical across both halves** — the discipline
...
> percentile method, `n`, and `n_dispositive`. ("Passes" is the probe set's
> word and does not transfer to a load profile.)
>
> **The schedule is fixed here and not at run time:** steps at 10, 25, 50, 75
> and 90 calls per second, 60 s per step, 15,000 calls per run. Changing it
...
> the window** — the nightly eval Lambda and `make evals` consume the same Titan
> RPM and Opus caps that select the dispositive step — **and the first run
...
> dispositive-step condition** — carrying per tier: p95 retrieval latency, the
...
> arrival rate driven), the vantage, the span status, and the verdict.
>
...
> within 5% of the driven rate for the whole step and neither tier recorded a
> Titan throttle in it. p95 is computed within that step, per tier; every other
...
> retrieve not at all — and each run's report states its verdict model and its
> token consumption against that model's daily cap. Neither profile carries
> Tier B's disposition.
...
- **SPEC/06's Observability section**; **ADR-0012 Rulings 1 and 2**; **leg 1 of
  ADR-0001**; **`evals/scenarios.json`** and what the three demo scenarios
  assert; **no golden question**.

## Alternatives considered

- **Run the `/query` profile anyway, shorter.** One 30-second Tier A hold is
  `30 × 189,981 = 5,699,419` Opus tokens — **2.2× the day's non-adjustable cap
...
## Corrections to v1 and v2, recorded rather than edited away
