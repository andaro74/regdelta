...
def retrieve_traced(query: str, filters: Filters,
    ...
    if endpoint:
        ...
        try:
            ...
            return _finish(raw, filters, k), Resolution("aoss", endpoint)
    ...
    return (_finish(raw, filters, k),
            Resolution("s3vectors", endpoint, fallback_reason))


def _finish(raw: list[Chunk], filters: Filters, k: int) -> list[Chunk]:
    """Re-apply filters client-side, then truncate.
    ...
    *means*, so a dialect that quietly matches everything (S3 Vectors on an
    ...
    return fusion.diversify(kept, config.RETRIEVAL_PER_DOC_CAP, k)
